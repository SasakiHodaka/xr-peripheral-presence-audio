...
- `F2`: speaker B
- `F3`: speaker C
- The response HUD also provides clickable direction and speaker buttons.

## Current Prototype Trial Flow

1. Open `Assets/Scenes/SceneTokenMock.unity`.
2. Run `Tools > Semantic Spatial Audio > Validate Scene Token Mock Scene`.
3. Press Play.
4. Confirm that the `Scene Tokens` HUD is visible.
5. Press `Space` to start the experiment session.
6. Let the scripted conversation run.
7. Confirm that the sequence includes:
   - one active speaker
   - question or answer semantic token
   - short overlap using two speakers
   - warning or instruction token
8. During each condition, record direction and speaker guesses using the
   response keys or HUD buttons.
9. Let timed condition advancement occur, or press `N`.
10. Stop after the final condition, or press `Space`.
11. Collect CSV logs from Unity's `Application.persistentDataPath`.
...
- response rows include `target`, `isCorrect`, `ambiguous`, and `responseLatency` fields
